...
def setup_argparse():
    ...
    parser.add_argument('--data-path', default='raw_data/wind3D/idealized/', 
                       help='Path to training data')
    parser.add_argument('--formatted-shape', type=int, default=1280,
                       help='Formatted shape for data preprocessing')
    parser.add_argument('--train-ratio', type=float, default=0.8,
                       help='Ratio of data to use for training')
    
    # Model arguments
    parser.add_argument('--chan-multi', type=int, default=20,
                       help='Channel multiplier for network')
    parser.add_argument('--max-level', type=int, default=6,
                       help='Maximum level of the U-Net')
    parser.add_argument('--activation', choices=['ReLU', 'PReLU'], default='ReLU',
...
def load_files_from_path(datapath):
    ...
    for path in paths:
        ...
        for (_, _, fnames) in walk(path):
            ...
            datafiles.extend([os.path.join(path, fname) for fname in fnames 
...
def parse_wind_angle(filename):
    ...
    if filename.endswith(('.nc', '.netcdf')):
        try:
            ...
            logger.warning("NetCDF support not available, cannot parse wind angle from NetCDF filename")
...
def save_training_results(data_fmt, files, angles, path, fmt_pred, indices, csv_name, saveimages=False):
...
def main():
        ...
        try:
            ...
            if f.endswith(('.nc', '.netcdf')):
                try:
                    ...
                    data = load_wind_data(f, netcdf_variable='wind_speed', z_level=1, transpose=False)
                    logger.info(f"Loaded NetCDF file {f} with shape {data.shape}")
                    images.append(data)
                except ImportError:
                    logger.error(f"NetCDF support not available. Please install netCDF4: pip install netCDF4")
                    sys.exit(1)
                except Exception as e:
                    # Try alternative variable names
                    try:
                        from feilian import load_wind_data
                        data = load_wind_data(f, netcdf_variable='Uped', z_level=1, transpose=False)
                        logger.info(f"Loaded NetCDF file {f} using 'Uped' variable with shape {data.shape}")
                        images.append(data)
    ...
    data_fmt = DataFormatter(images, wind_angles=angles, formatted_shape=args.formatted_shape)
    
    # Split data
    x_train, y_train, train_idx, x_test, y_test, test_idx = data_fmt.split_train_test_data(
    ...
    train_metrics_csv = f"metrics_train_seed{args.seed}_act{args.activation}_time{curr_time}.csv"
    save_training_results(
        data_fmt, files, angles, train_images_dir, y_train_pred, train_idx, 
        train_metrics_csv, args.save_images
    )
    
    # Test results
    test_images_dir = f"{args.output_dir}/images/{case_type}/test"
    test_metrics_csv = f"metrics_test_seed{args.seed}_act{args.activation}_time{curr_time}.csv"
    save_training_results(
        data_fmt, files, angles, test_images_dir, y_test_pred, test_idx, 
        test_metrics_csv, args.save_images
    )
    ...
    if device_manager.device.type == "cuda":
        import torch
        logger.info(f"Final GPU memory usage: {torch.cuda.memory_allocated(device_manager.device) / 1e9:.1f} GB")
