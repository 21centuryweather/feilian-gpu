...
def test_z_level_extraction(sample_netcdf_file):
    ...
    assert z1_data.dtype == np.float32, (
        f"z=1 data should be float32, got {z1_data.dtype}"
    )
    ...
    assert nonzero_count > 0, (
        f"z=1 should contain non-zero wind speeds, got {nonzero_count}/{z1_data.size} non-zero values"
    )
    ...
    assert z0_data.dtype == np.float32, (
        f"z=0 data should be float32, got {z0_data.dtype}"
    )
    assert z0_data.shape == z1_data.shape, (
        f"z=0 and z=1 should have same shape, got {z0_data.shape} vs {z1_data.shape}"
    )

    # Content validation - surface level should typically be zeros
    z0_nonzero_count = np.count_nonzero(z0_data)
    assert z0_nonzero_count == 0, (
        f"z=0 slice should be all zeros, got {z0_nonzero_count}/{z0_data.size} non-zero values"
    )

    # Statistical validation
    assert z1_data.min() >= 0.0, (
        f"Wind speeds should be non-negative, got min={z1_data.min()}"
    )
    assert z1_data.max() > 0.0, (
        f"Should have some positive wind speeds, got max={z1_data.max()}"
    )

    print(
        f"✓ z=1 data shape: {z1_data.shape}, range: [{z1_data.min():.6f}, {z1_data.max():.6f}]"
    )
...
def test_universal_loader(sample_netcdf_file):
    ...
    data = load_wind_data(
        str(sample_netcdf_file), netcdf_variable="wind_speed", z_level=1
    )
    ...
    print(
        f"✓ Universal loader: shape {data.shape}, {np.count_nonzero(data)}/{data.size} non-zero values"
    )
...
def test_netcdf_info_extraction(sample_netcdf_file):
    ...
    assert len(wind_speed_var["shape"]) == 3, (
        f"Wind speed should be 3D, got shape {wind_speed_var['shape']}"
    )
    assert wind_speed_var["dimensions"] == ("x", "y", "z"), (
        f"Should have (x, y, z) dimensions, got {wind_speed_var['dimensions']}"
    )

    # Check dimension sizes are reasonable
    x_size, y_size, z_size = wind_speed_var["shape"]
    assert x_size > 0 and y_size > 0 and z_size > 0, (
        f"All dimensions should be positive: {wind_speed_var['shape']}"
    )
    assert z_size >= 2, (
        f"Should have at least 2 z-levels (surface + ground), got {z_size}"
    )

    print(
        f"✓ NetCDF structure: {wind_speed_var['dimensions']} with shape {wind_speed_var['shape']}"
    )
...
def test_variable_name_fallback(sample_netcdf_file):
    ...
    try:
        data = load_netcdf_wind_speed(
            str(sample_netcdf_file), variable_name="nonexistent_var", z_level=1
        )
