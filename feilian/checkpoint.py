...
class CheckpointPolicyInterface(ABC):
    ...
    def should_checkpoint(
        self, layer: nn.Module, layer_idx: int, memory_usage: Optional[float] = None
    ) -> bool:
...
class EveryNPolicy(CheckpointPolicyInterface):
    ...
    def __init__(
        self, interval: int = 2, preserve_rng: bool = True, use_reentrant: bool = True
    ):
...
class CustomPolicy(CheckpointPolicyInterface):
    ...
    def __init__(
        self, should_checkpoint_fn: Callable, checkpoint_fn: Optional[Callable] = None
    ):
...
class CheckpointWrapper(nn.Module):
    ...
    def __init__(
        self, module: nn.Module, policy: CheckpointPolicyInterface, layer_idx: int = 0
    ):
        super().__init__()
        self.module = module
        self.policy = policy
    ...
    def __init__(self, *modules, policy: CheckpointPolicyInterface):
        ...
        for i, module in enumerate(modules):
            ...
            if policy.should_checkpoint(module, i):
                ...
                logger.debug(
                    f"Checkpointing enabled for layer {i}: {type(module).__name__}"
                )
            else:
...
def _wrap_recursive(
    ...
    def _apply_checkpointing(module: nn.Module) -> nn.Module:
        ...
        if policy.should_checkpoint(module, layer_idx):
            logger.debug(
                f"Applying checkpointing to {type(module).__name__} at layer {layer_idx}"
            )
    ...
    if hasattr(model, "_modules") and isinstance(model._modules, dict):
        ...
        for i, (name, module) in enumerate(model._modules.items()):
            ...
            if policy.should_checkpoint(module, i):
                ...
                logger.debug(
                    f"Checkpointing enabled for {name}: {type(module).__name__}"
                )
            else:
                new_modules.append(module)
...
def analyze_checkpointing_opportunities(
    model: nn.Module, memory_threshold_mb: float = 50.0
) -> Dict[str, Any]:
    """
