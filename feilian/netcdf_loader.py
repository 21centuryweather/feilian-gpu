...
try:
    ...
    nc = None
    if not XARRAY_AVAILABLE:
        logger.warning("Neither xarray nor netCDF4 available. Install with: pip install xarray netcdf4")
...
def load_netcdf_wind_speed_xarray(filepath: str,
    ...
    """
    if not XARRAY_AVAILABLE:
        raise ImportError("xarray library is required. Install with: pip install xarray")
    ...
    try:
        ...
        with xr.open_dataset(filepath, decode_times=False) as ds:
            ...
            if variable_name not in ds.data_vars:
                ...
                alt_names = ['wind_speed', 'Uped', 'u', 'velocity', 'speed', 'ws', 'U', 'V']
                found_var = None
            ...
            if data_var.ndim == 2:
                ...
                if has_duplicate_dims:
                    ...
                    if z_level < data_var.shape[0]:
                        try:
                            ...
                            logger.info(f"Extracted z={z_level} slice using positional indexing (first dimension)")
                        except Exception as e:
                            logger.warning(f"Failed to extract slice with positional indexing: {e}")
                            # Fallback: try other dimension patterns
                            if z_level < data_var.shape[1]:
                                result = data_var.values[:, z_level, :]
                                logger.info(f"Extracted z={z_level} slice using second dimension")
                            elif z_level < data_var.shape[2]:
                                result = data_var.values[:, :, z_level]
                                logger.info(f"Extracted z={z_level} slice using third dimension")
                            else:
                                # Use first slice as fallback
                                result = data_var.values[0, :, :]
                                logger.warning(f"Using first slice as fallback")
                    else:
                        max_z = data_var.shape[0] - 1
                        logger.warning(f"z_level {z_level} exceeds maximum {max_z}, using {max_z}")
                        result = data_var.values[max_z, :, :]
                        
                else:
                    # Normal case with distinct dimension names
                    z_dim_names = ['z', 'height', 'level', 'vertical', 'lev', 'alt', 'altitude']
                    z_dim = None
                    ...
                    if z_dim is not None:
                        ...
                        if z_level < data_var.sizes[z_dim]:
                            ...
                            logger.info(f"Extracted z={z_level} slice using dimension '{z_dim}'")
                        else:
                        ...
                        logger.warning("No clear z dimension found, assuming first dimension is z")
                        if z_level < data_var.shape[0]:
                            result = data_var.isel({data_var.dims[0]: z_level}).values
                            logger.info(f"Extracted z={z_level} slice from first dimension")
                        else:
                            max_z = data_var.shape[0] - 1
                            logger.warning(f"z_level {z_level} exceeds maximum {max_z}, using {max_z}")
                ...
                if not selection:
                    ...
                    logger.warning("Could not identify time/z dimensions, using first slice")
                    result = data_var.isel({data_var.dims[0]: 0, data_var.dims[1]: z_level}).values
                else:
                    result = data_var.isel(selection).values
                
            else:
                raise ValueError(f"Unsupported data dimensions: {data_var.ndim}D. Expected 2D, 3D, or 4D data.")
            ...
            if result.ndim != 2:
                raise ValueError(f"Expected 2D result after extraction, got {result.ndim}D with shape {result.shape}")
            
            # Check for empty or scalar results
            if result.size == 0:
                raise ValueError(f"Extracted data is empty. File might be corrupted or have wrong structure.")
            
            if result.shape == ():
                raise ValueError(f"Extracted data is scalar. Expected 2D array but got single value.")
            
            # Handle NaN values - replace with 0 to prevent training issues
            nan_count = np.sum(np.isnan(result))
            if nan_count > 0:
                logger.info(f"Replacing {nan_count} NaN values with 0.0 to prevent training issues")
...
def load_netcdf_wind_speed_netcdf4(filepath: str,
                                   variable_name: str = 'wind_speed',
                                   z_level: int = 1,
                                   transpose: bool = False) -> np.ndarray:
    """
    ...
    if not NETCDF4_AVAILABLE:
        raise ImportError("netCDF4 library is required. Install with: pip install netcdf4")
    ...
    try:
        with nc.Dataset(filepath, 'r') as dataset:
            ...
            try:
                ...
                try:
                    test_slice = wind_data[0, :, :] if len(wind_data.shape) >= 3 else wind_data[:]
                    logger.warning(f"File {filepath} has access issues, attempting slice-by-slice loading")
                    ...
                    if len(wind_data.shape) == 3:
                        ...
                        logger.info(f"Using z=0 slice directly due to file access issues")
                    else:
                        data_array = test_slice
                except Exception as e2:
                    logger.error(f"Cannot access data in {filepath}: {e2}")
                    raise ValueError(f"Corrupted NetCDF file: cannot access wind speed data. Original error: {e}")
            ...
            if data_array.ndim == 2:
                ...
                raise ValueError(f"Unsupported data dimensions: {data_array.ndim}D. Expected 2D or 3D data.")
...
def load_netcdf_wind_speed(filepath: str, 
                          variable_name: str = 'wind_speed',
                          z_level: int = 1,
                          transpose: bool = False,
                          prefer_xarray: bool = True) -> np.ndarray:
    """
    ...
    if prefer_xarray and XARRAY_AVAILABLE:
        ...
        try:
            return load_netcdf_wind_speed_xarray(filepath, variable_name, z_level, transpose)
        except Exception as e:
            if NETCDF4_AVAILABLE:
                logger.warning(f"xarray failed ({e}), falling back to netCDF4")
                return load_netcdf_wind_speed_netcdf4(filepath, variable_name, z_level, transpose)
            else:
                raise
...
def load_wind_data(filepath: str, 
...
def find_wind_data_files(data_path: str, 
                        include_netcdf: bool = True, 
                        include_numpy: bool = True) -> List[str]:
    """
...
def convert_netcdf_to_numpy(netcdf_path: str, 
                           output_dir: str,
                           variable_name: str = 'wind_speed',
                           z_level: int = 0,
...
def batch_convert_netcdf_to_numpy(netcdf_dir: str,
                                 output_dir: str, 
                                 variable_name: str = 'wind_speed',
                                 z_level: int = 0,
                                 transpose: bool = False) -> List[str]:
    """
    ...
    """
    netcdf_files = find_wind_data_files(netcdf_dir, include_netcdf=True, include_numpy=False)
    converted_files = []
...
def get_netcdf_info(filepath: str) -> Dict[str, Any]:
    ...
    """
    if not NETCDF_AVAILABLE:
        raise ImportError("NetCDF support is required. Install with: pip install xarray netcdf4")
        ...
        with nc.Dataset(filepath, 'r') as dataset:
            info = {
                'variables': {
                    ...
                    } for name, var in dataset.variables.items() if name not in dataset.dimensions
                },
                'coordinates': {
                    name: {
                        'shape': var.shape,
                        'dtype': str(var.dtype),
                        'attributes': {attr: getattr(var, attr) for attr in var.ncattrs()}
                    } for name, var in dataset.variables.items() if name in dataset.dimensions
                },
...
def load_multiple_variables(filepath: str,
    ...
    if XARRAY_AVAILABLE:
        ...
        with xr.open_dataset(filepath, decode_times=False) as ds:
            for var_name in variable_names:
                if var_name not in ds.data_vars:
                    available_vars = list(ds.data_vars.keys())
                    raise KeyError(f"Variable '{var_name}' not found. Available variables: {available_vars}")
                
                # Load the variable using the same logic as single variable loading
                try:
                    data = load_netcdf_wind_speed_xarray(filepath, var_name, z_level, transpose)
        ...
        for var_name in variable_names:
            try:
                data = load_netcdf_wind_speed_netcdf4(filepath, var_name, z_level, transpose)
                results[var_name] = data
            except Exception as e:
                logger.error(f"Failed to load variable '{var_name}': {e}")
                raise
...
def load_netcdf_time_series(filepath: str,
                            variable_name: str = 'wind_speed',
                            z_level: int = 1,
    ...
    try:
        with xr.open_dataset(filepath, decode_times=False) as ds:
            ...
            if data_var.ndim < 3:
                raise ValueError("Time series loading requires at least 3D data (time, y, x) or 4D data (time, z, y, x)")
