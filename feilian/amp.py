...
import warnings
from typing import Optional, Dict, Any, Callable, Union, List, Set, Tuple
...
class AMPConfig:
    ...
    autocast_cpu_ops: Set[str] = field(default_factory=lambda: {
        'conv1d', 'conv2d', 'conv3d', 'bmm', 'mm', 'mv', 'linear',
...
class GradScaler:
    ...
    def load_state_dict(self, state_dict: Dict[str, Any]) -> None:
        ...
        self._inf_has_been_found = state_dict.get('inf_has_been_found', 
        ...
        try:
            for op_name in ops_to_patch:
                ...
                if hasattr(torch.nn.functional, op_name):
                    ...
                    setattr(torch.nn.functional, op_name, 
...
class AMPManager:
    ...
    def get_scaler(self, device: Union[str, torch.device] = 'auto') -> Optional[GradScaler]:
    ...
    def autocast(self, 
                ...
                dtype: Optional[torch.dtype] = None,
                enabled: Optional[bool] = None) -> AutocastContext:
        ...
        if dtype is None:
            if self.config.precision_mode == PrecisionMode.FP16:
                ...
                if target_device.type == 'cuda':
                    ...
                    dtype = torch.bfloat16 if torch.cpu.is_bf16_supported() else torch.float16
                else:  # MPS
                    dtype = torch.float16
    ...
    def get_effective_batch_size(self, base_batch_size: int, device: Union[str, torch.device] = 'auto') -> int:
...
@contextmanager
def autocast(device: Union[str, torch.device] = 'auto',
            dtype: Optional[torch.dtype] = None,
...
def scale_loss(loss: torch.Tensor, device: Union[str, torch.device] = 'auto') -> torch.Tensor:
    """
...
def backward_and_step(loss: torch.Tensor,
                     optimizer: torch.optim.Optimizer,
                     device: Union[str, torch.device] = 'auto',
                     max_norm: Optional[float] = None) -> bool:
    """
