...
class ModelFactory:
    ...
    def create_transformer(
        vocab_size: int = 32000, d_model: int = 512, nhead: int = 8, num_layers: int = 6
    ) -> nn.Module:
...
class BenchmarkRunner:
    ...
    def benchmark_model(
        ...
        # Create sample data
        input_data = self._create_sample_data(
            model_name, batch_size, sequence_length, device
        )
        ...
        if model_name == "cnn":
            ...
            target = torch.randint(
                0, 32000, (batch_size, sequence_length), device=device
            )
        else:  # diffusion
        ...
        avg_forward_time = sum(forward_times[stable_start:]) / len(
            forward_times[stable_start:]
        )
        avg_backward_time = sum(backward_times[stable_start:]) / len(
            backward_times[stable_start:]
        )
        avg_total_time = sum(total_times[stable_start:]) / len(
            total_times[stable_start:]
        )
        ...
        for device_pref in devices:
            for model_name in models:
                for batch_size in batch_sizes:
                    try:
                        print(
                            f"Benchmarking {model_name} with batch_size={batch_size} on {device_pref}"
                        )
                        ...
                        print(
                            f"Failed to benchmark {model_name} with batch_size={batch_size} on {device_pref}: {e}"
                        )
                        continue
...
def create_benchmark_cli():
    ...
    parser.add_argument(
        "--models",
        nargs="+",
        choices=["cnn", "transformer", "diffusion"],
    ...
    parser.add_argument(
        "--iterations", type=int, default=50, help="Number of iterations per benchmark"
    )
...
def main():
    ...
    results = runner.run_comprehensive_benchmark(
        models=args.models, batch_sizes=args.batch_sizes, devices=args.devices
    )
    ...
    if results:
        ...
        print(
            f"  Throughput: {best_throughput.throughput_samples_per_sec:.2f} samples/sec"
        )
