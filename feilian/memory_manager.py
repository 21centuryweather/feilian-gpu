...
class TensorLifecycleTracker:
    ...
    def register_allocation(self, tensor: torch.Tensor) -> None:
        ...
        with self._lock:
            ...
            current_allocated = sum(
                alloc.size_bytes for alloc in self._allocations.values() 
                if str(alloc.device) == device_key
            )
            self._peak_memory[device_key] = max(
                self._peak_memory[device_key], 
                current_allocated
            )
    ...
    def get_memory_stats(self, device: Optional[torch.device] = None) -> MemoryStats:
        ...
        with self._lock:
            if device is None:
                # Global stats
                total_allocated = sum(info.size_bytes for info in self._allocations.values())
                total_count = len(self._allocations)
                peak_allocated = max(self._peak_memory.values()) if self._peak_memory else 0
                device_key = "global"
            else:
                device_key = str(device)
                device_allocations = [
                    info for info in self._allocations.values() 
...
class CPUMemoryAllocator(BaseMemoryAllocator):
    ...
    def get_memory_stats(self) -> MemoryStats:
        ...
        return MemoryStats(
            ...
            peak_allocated_bytes=memory_info.peak_wset if hasattr(memory_info, 'peak_wset') else memory_info.rss,
            fragmentation_ratio=pool_memory / memory_info.rss if memory_info.rss > 0 else 0.0
        )
...
class CUDAMemoryAllocator(BaseMemoryAllocator):
    ...
    def allocate(self, shape: Tuple[int, ...], dtype: torch.dtype = torch.float32, 
        ...
        with self._lock:
            ...
            self._current_stream_idx = (self._current_stream_idx + 1) % len(self._streams)
...
class MemoryManager:
    ...
    def allocate(self, shape: Union[Tuple[int, ...], torch.Size], 
    ...
    def get_memory_stats(self, device: Union[str, torch.device, None] = None) -> MemoryStats:
        """
    ...
    def set_memory_pool_size(self, size_mb: int, device: Union[str, torch.device, None] = None) -> None:
        """
...
def allocate(shape: Union[Tuple[int, ...], torch.Size], 
            dtype: torch.dtype = torch.float32, 
            device: Union[str, torch.device, None] = None,
            requires_grad: bool = False) -> torch.Tensor:
    """
